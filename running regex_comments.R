...
# extract laps
laps_1 <- str_extract(original$V1,"\\d+([\\.\\,]\\d)*\\s*kr")
...
rownames(all_data) <- NULL #cleaning up
...
dates_kc1 <- dates_2[kc_1]
dates_kc2 <- dates_2[kc_1 + 1]
dates_kc3 <- dates_2[kc_1 + 2]
...
dates_zg1 <- dates_2[zg_1]
dates_zg2 <- dates_2[zg_1 + 1]
dates_zg3 <- dates_2[zg_1 + 2]
...
# Apply that to everything as the default. It will be changed where needed.
...
# It works! and now the main part...
